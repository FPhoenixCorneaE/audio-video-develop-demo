...
import com.fphoenixcorneae.audio.ui.MyApp
...
fun playMedia(rawId: Int) {
    CoroutineScope(Dispatchers.IO).launch {
        kotlin.runCatching {
            mMediaPlayer = MediaPlayer.create(MyApp.getInstance(), rawId).apply {
...
fun playMedia(data: ByteArray, audioType: String) {
    runCatching {
        if (data.isNotEmpty() && audioType.isNotEmpty()) {
            val tempMp3 = File(MyApp.getInstance().cacheDir, "temp$audioType")
